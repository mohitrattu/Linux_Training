		...
		switch( fret)
        	{
				...
				//write result on client message queue
				msg.msg_type = MSG_TYPE;
				msg.res = *(infra->res);

				count = msgsnd(infra->msg_id,&msg,sizeof(msg),0);
				shmdt(infra->res);

				printf("File:%s; Func:%s. Result: %d Bytes\n",__FILE__,__func__,msg.res.result);
